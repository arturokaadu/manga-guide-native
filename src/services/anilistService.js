...
export async function searchAnimeList(query) {
  ...
  try {
    ...
    results.forEach(anime => {
      const rawTitle = anime.title.romaji || anime.title.english;

      // aggressive cleaning
      const cleanTitle = rawTitle
        .replace(/(:?\s*Season\s*\d+|:?\s*\d+(st|nd|rd|th)\s*Season)/gi, '')
        .replace(/(:?\s*Part\s*\d+)/gi, '')
        .replace(/(:?\s*Cour\s*\d+)/gi, '')
        .replace(/(:?\s*Shibuya\s*Incident.*)/gi, '') // Specific JJK fix
        .replace(/(:?\s*Entertainment\s*District.*)/gi, '') // Demon Slayer fix
        .trim();
      ...
      if (!seen.has(normalized)) {
        seen.add(normalized);
        // Return object with CLEAN title for display
        filtered.push({
          ...anime,
          title: { ...anime.title, userPreferred: cleanTitle }
        });
      }
    });

    return filtered.slice(0, 8); // Limit to 8 unique results
  } catch (error) {
...
export async function getTrendingAnime() {
  ...
  try {
    ...
    const data = await response.json();
    const rawTrending = data.data.Page.media || [];

    return rawTrending.map(anime => {
      const rawTitle = anime.title.romaji || anime.title.english;
      const cleanTitle = rawTitle
        .replace(/(:?\s*Season\s*\d+|:?\s*\d+(st|nd|rd|th)\s*Season)/gi, '')
        .replace(/(:?\s*Part\s*\d+)/gi, '')
      ...
      return {
        ...anime,
        title: { ...anime.title, romaji: cleanTitle } // Override for display
      };
    });
  } catch (error) {
